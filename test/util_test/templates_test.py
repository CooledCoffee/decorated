...
class CompileTest(TestCase):
    ...
    def test_begin_ends_with_vars(self):
        ...
        self.assertEqual('a', template._parts[0].expression)
        self.assertIsInstance(template._parts[1], VariablePart)
        self.assertEqual('b', template._parts[1].expression)
        self.assertIsInstance(template._parts[2], VariablePart)
        self.assertEqual('c', template._parts[2].expression)
        
    def test_attr(self):
        template = templates.compile('{user.id}', ['user'])
        self.assertEqual('user.id', template._parts[0].expression)
